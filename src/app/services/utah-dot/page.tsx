    ...
    <main className="min-h-screen">
      ...
      <LakesideShell>
        ...
        <section className="py-14 md:py-24 lg:py-[120px]">
          <div className={frame}>
            <h2 className={`${h2Class} text-lake-ink mb-3`}>All 28 Rest Area Locations</h2>
            <p className="text-lake-mute mb-4">Interactive map of every rest area managed by BA Services across Utah. Click any pin for details.</p>
            <div className="flex flex-wrap gap-4 mb-6">
              {regionInfo.map((r) => (
                <div key={r.id} className="flex items-center gap-2 text-sm text-lake-mute">
        ...
        {regionInfo.map((region) => {
          ...
          return (
            <section key={region.id} className={`py-16 ${region.id % 2 === 0 ? bandTint : bandWhite}`}>
              <div className={frame}>
                <div className="flex items-center gap-3 mb-2">
                  <span className={`w-4 h-4 rounded-full ${regionColorClasses[region.id].dot}`} />
                  <h2 className="font-lake-serif text-[26px] text-lake-ink">{region.name}</h2>
                </div>
                <p className="text-lake-mute mb-8 max-w-3xl">{region.description}</p>
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {areas.map((ra) => (
                    <div key={`${ra.name}-${ra.direction}`} className="rounded-md border border-lake-line bg-white p-5">
                      <div className="flex items-start justify-between mb-3 gap-2">
                        <h3 className="font-semibold text-lake-ink text-sm">{ra.name}</h3>
                        {ra.direction && (
                      ...
                      <div className="space-y-1 text-xs text-lake-mute">
                      ...
                      <a
                        ...
                        className="inline-flex min-h-[44px] items-center gap-1 mt-3 text-xs font-medium text-lake-ink underline decoration-lake-line decoration-1 underline-offset-[6px] hover:decoration-lake-ink"
                      >
                        Directions
        ...
        <section className={`py-14 md:py-24 lg:py-[120px] ${bandTint}`}>
          <div className={frame}>
            <div className="max-w-3xl mb-12">
              ...
              <h2 className={`${h2Class} text-lake-ink mb-4`}>Contract Scope of Services</h2>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              {scopeOfWork.map((category) => (
                <div key={category.title} className="rounded-md border border-lake-line bg-white p-6">
                  <h3 className="font-lake-serif text-[20px] text-lake-ink mb-2">{category.title}</h3>
                  <p className="text-sm text-lake-mute mb-4">{category.description}</p>
