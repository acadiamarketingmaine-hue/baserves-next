...
const SLUG = 'tipsaw-lake-recreation-area'
...
// The words and photos live in src/content/defaults/tipsaw-lake-recreation-area.ts.
...
export default async function TipsawLakePage() {
  const content = (await getPropertyContent(SLUG))!
  const fishSpecies = content.sections.fishing.items ?? []
  const facilities = content.sections.facilities.items ?? []
  const additionalFacilities = content.sections.additionalFacilities.items ?? []
    ...
    <main className="min-h-screen">
      ...
      <section className="relative h-[70vh] min-h-[500px] flex items-end">
        ...
        <div className="relative z-10 container-custom px-6 pb-16">
          ...
          <Link href={content.ctas.parentForest.url} className="inline-flex items-center text-white/70 hover:text-white transition-colors mb-6 text-sm">
          ...
          <div className="grid lg:grid-cols-3 gap-12">
            <div className="lg:col-span-2">
              ...
              <div className="prose prose-lg max-w-none">
                ...
                <h3 className="text-2xl font-bold text-gray-900 mt-8 mb-4">{content.sections.campgroundFacilities.heading}</h3>
                <p className="text-gray-600 leading-relaxed mb-4">
                ...
                <p className="text-gray-600 leading-relaxed mb-4">
                  {content.sections.campgroundFacilities.paragraphs?.[2]}
                </p>
                <p className="text-gray-600 leading-relaxed mb-4">
                  {content.sections.campgroundFacilities.paragraphs?.[3]}
                </p>

                <h3 className="text-2xl font-bold text-gray-900 mt-8 mb-4">{content.sections.dayUseAmenities.heading}</h3>
                <p className="text-gray-600 leading-relaxed mb-4">
                  {content.sections.dayUseAmenities.paragraphs?.[0]}
                </p>
                <p className="text-gray-600 leading-relaxed mb-4">
                  {content.sections.dayUseAmenities.paragraphs?.[1]}
                </p>

                <h3 className="text-2xl font-bold text-gray-900 mt-8 mb-4">{content.sections.trailsAndRecreation.heading}</h3>
                <p className="text-gray-600 leading-relaxed mb-4">
                  {content.sections.trailsAndRecreation.paragraphs?.[0]}
                </p>

                <h3 className="text-2xl font-bold text-gray-900 mt-8 mb-4">{content.sections.accessAndOperations.heading}</h3>
                <p className="text-gray-600 leading-relaxed mb-4">
              ...
              <div className="mt-10 bg-blue-50 rounded-2xl p-8">
                <h3 className="text-2xl font-bold text-gray-900 mb-4">{content.sections.fishing.heading}</h3>
                <p className="text-gray-600 leading-relaxed mb-4">
                  {content.sections.fishing.paragraphs?.[0]}
                </p>
                <div className="flex flex-wrap gap-3">
                  {fishSpecies.map((fish) => (
                    <span key={fish.key} className="inline-flex items-center gap-2 px-4 py-2 bg-white rounded-full text-sm font-medium text-blue-800">
            ...
            {additionalFacilities.map((facility) => (
              <div key={facility.key} className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                <div className="relative h-40">
                  <Image src={facility.photo!.src} alt={facility.photo!.alt} fill sizes="(min-width: 1280px) 400px, (min-width: 768px) 33vw, 100vw" className="object-cover" />
                </div>
                <div className="p-5">
                  <h4 className="text-lg font-bold text-gray-900 mb-1">{facility.title}</h4>
