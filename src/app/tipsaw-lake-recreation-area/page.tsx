...
// src/content/defaults/tipsaw-lake-recreation-area.ts. Layout is the
// Lakeside template.
export default async function TipsawLakePage() {
  ...
  const facilities = [...(sections.facilities.items ?? []), ...(sections.additionalFacilities.items ?? [])]
  const fishSpecies = (sections.fishing.items ?? []).map((f) => f.title ?? '').filter(Boolean)
  ...
  return (
    <main className="min-h-screen">
      ...
      <LakesideShell>
        ...
        {/* Camp details: campground facilities, day-use amenities, trails,
            fishing, access and operations — the page's own "Overview"
            content, restyled and stacked, with the second and last ember
            rule marking this fresh start. */}
        <section className={`${block} ${bandWhite} pt-14 md:pt-24 lg:pt-[120px]`}>
          <div className={frame}>
            ...
            <h3 className={`${h3} mt-16 mb-4 text-lake-ink`}>{sections.dayUseAmenities.heading ?? 'Day-Use Amenities'}</h3>
            <div className="grid gap-5 md:grid-cols-2 md:gap-x-12">
              {(sections.dayUseAmenities.paragraphs ?? []).map((p) => (
                <p key={p} className={body}>
                  {p}
                </p>
              ))}
            ...
            <h3 className={`${h3} mt-16 mb-4 text-lake-ink`}>{sections.trailsAndRecreation.heading ?? 'Trails and Recreation'}</h3>
            <div className="grid gap-5 md:grid-cols-2 md:gap-x-12">
              {(sections.trailsAndRecreation.paragraphs ?? []).map((p) => (
                <p key={p} className={body}>
                  {p}
                </p>
              ))}
            ...
            <h3 className={`${h3} mt-16 mb-4 text-lake-ink`}>{sections.fishing.heading ?? 'Fishing at Tipsaw Lake'}</h3>
            <div className="grid gap-5 md:grid-cols-2 md:gap-x-12">
              {(sections.fishing.paragraphs ?? []).map((p) => (
                <p key={p} className={body}>
                  {p}
                </p>
              ))}
            </div>
            {fishSpecies.length > 0 && <IconChipList items={fishSpecies} className="mt-6" />}
        ...
        <section className={`${block} ${bandTint} pt-14 md:pt-24 lg:pt-[120px]`}>
          <div className={frame}>
            ...
            <CardRow columns={3} items={facilities.map((f) => ({ key: f.key, title: f.title, body: f.body, photo: f.photo }))} />
        ...
        <ClosingCta
          photo={{ src: '/images/tipsaw-lake/campsite2.jpg', alt: 'RV campsites along a paved campground loop at Tipsaw Lake' }}
